...
class PlayRoomsSeedCommand(val plugin: LobbyPlugin) : CommandExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        if (args.size < 2) {
            sender.sendMessage("§cUsage: /playroomsseed <seed> <amount of rooms>")
            return true
        }

        try {
            val passedSeed = args[0].toLongOrNull() ?: return false
            val rooms = args[1].toIntOrNull() ?: return false
            val seed = plugin.generateSeed(passedSeed, rooms)
